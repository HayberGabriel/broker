...
class Broker:
    ...
    def get_topics(self):
        first_client = list(broker.clients.values())[0]
        return first_client.get_topics()
    
    def get_messages_from_topic(self, topic_name):
        first_client = list(broker.clients.values())[0]
        
        if first_client:
            if topic_name in first_client.topics:
                return first_client.topics[topic_name]
...
class Client:
    def __init__(self, name):
        ...
        self.topics = {"Tópico Inicial": ["Mensagem 1", "Mensagem 2"]}
...
class AdminApp:
    ...
    def list_topic_messages(self):
        ...
        if dialog.result:
            topic_name = dialog.result
            messages = broker.get_messages_from_topic(topic_name)
